...
(ADMIN_BROADCAST, ADMIN_TRACKING_ID, ADMIN_TRACKING_CODE, ADMIN_DISCOUNT_ID, ADMIN_DISCOUNT_MSG) = range(10, 15)
(PROFILE_NAME, PROFILE_BIRTHDAY) = range(20, 22)

# ════════════════════════════════════════════════════════════
# هوش مصنوعی با Gemini API جدید
# ════════════════════════════════════════════════════════════
...
- شماره اضطراری: 09040710470

محصولات: لوازم آرایشی، لوازم خانه، ورزشی، مردانه، زنانه، بچه‌گانه، کیف (همه نوع)، کفش (همه نوع)
کشورها: امارات (عمده)، ترکیه، کانادا، آمریکا

زمان تحویل:
- ترکیه: ۲ تا ۴ هفته (شرایط فعلی - مشکل پرواز)
- امارات: ۴ تا ۷ هفته (شرایط فعلی - مشکل ارسال)
- در زمان حراج ممکنه بیشتر بشه

هزینه باربری امارات:
- لباس و اکسسوری: هر ۱۰۰گرم = ۹۹,۰۰۰ تومان
- کفش: جفتی ۶۵۰,۰۰۰ تومان
- کیف: دانه‌ای ۶۰۰,۰۰۰ تومان

هزینه باربری ترکیه: هر ۱۰۰گرم = ۵۸,۰۰۰ تومان (هوایی)

پرداخت: کارت به کارت - شماره کارت از ادمین بگیرید
باربری بعد از رسیدن سفارش اعلام میشه

تعویض و کنسلی نداریم. اگه آسیب دیده یا اشتباه بود تا ۴۸ ساعت عکس بفرست.
موجودی: اگه «موجود در ایران» نوشته = موجود، وگرنه = سفارشیه

قوانین پاسخ:
- صمیمی و دوستانه باش، از ایموجی استفاده کن
- برای قیمت محصول بگو از ادمین بپرسن
- اگه جواب نمیدونی بگو: پیام بده ادمین جواب میده
...
async def order_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🛍️ *ثبت سفارش شایلی*\n\nاز کدوم کشور می‌خوای سفارش بدی؟", parse_mode="Markdown", reply_markup=country_keyboard())
...
async def order_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ حالا *محصولی که می‌خوای* رو توضیح بده:\n_(لینک، عکس، اسم، رنگ، سایز)_", parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    return ORDER_PRODUCT

async def order_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        context.user_data["order"]["product"] = f"[عکس - {update.message.photo[-1].file_id}]"
        if update.message.caption:
            context.user_data["order"]["product"] += f" | {update.message.caption}"
    else:
        context.user_data["order"]["product"] = update.message.text
    await update.message.reply_text("👌 *نام و نام خانوادگی* کاملت:", parse_mode="Markdown")
    return ORDER_NAME
...
async def order_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (phone.startswith("09") and len(phone) == 11 and phone.isdigit()):
        await update.message.reply_text("⚠️ شماره باید با ۰۹ شروع بشه و ۱۱ رقم باشه:")
        return ORDER_PHONE
...
async def order_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        receipt_info = f"[فایل - {update.message.document.file_id}]"
    else:
        receipt_info = update.message.text or "نامشخص"
    ...
    chat_id = update.effective_user.id
    order = context.user_data["order"]
    order.update({"receipt": receipt_info, "order_id": order_id, "date": datetime.now().strftime("%Y-%m-%d %H:%M"), "status": "در انتظار بررسی", "chat_id": chat_id})
    ...
    try:
        await context.bot.send_message(chat_id=ADMIN_CHAT_ID, parse_mode="Markdown",
            text=f"🔔 *سفارش جدید شایلی!*\n\n🆔 کد: `{order_id}`\n👤 {user_tg.first_name} (@{user_tg.username or '-'})\n🆔 Chat: `{chat_id}`\n\n🌍 {order['country']}\n📦 {order['product']}\n👤 {order['name']}\n📱 {order['phone']}\n📍 {order['address']}\n📮 {order['postal']}\n🧾 {receipt_info}")
    except Exception as e:
        logger.error(f"Admin notify: {e}")

    await update.message.reply_text(
        f"✅ *سفارش ثبت شد!*\n\n🎉 کد سفارش: `{order_id}`\nاین کد رو نگه‌دار 📌\n\n━━━━━━━━━━━━━━\n📣 برای دسترسی در هر شرایطی، کانال بله ما رو دنبال کن:\n{BALE_LINK}",
...
async def tracking_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user.get("orders"):
        await update.message.reply_text("📦 سفارشی ثبت نشده.\nکد سفارشت رو بنویس تا چک کنم 👇")
        return
    msg = "📦 *سفارش‌های شما:*\n\n"
    for oid in user.get("orders", [])[-5:]:
        for o in db["orders"]:
            if o.get("order_id") == oid:
                msg += f"🔖 `{oid}` | {o.get('date','-')} | {o.get('status','-')}\n"
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ فرستاده شد!\n✔️ موفق: {sent} | ❌ ناموفق: {failed}", reply_markup=admin_keyboard())
    return ConversationHandler.END
...
async def tracking_admin_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for o in db["orders"]:
        if o.get("order_id") == order_id:
            ...
            break
    save_db(db)
    msg = f"✅ کد پیگیری ثبت شد و به مشتری اطلاع داده شد!" if found else f"❌ سفارش {order_id} پیدا نشد."
...
async def discount_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await context.bot.send_message(chat_id=int(context.user_data["discount_chat_id"]),
            text=f"🎁 *پیام ویژه از شایلی:*\n\n{update.message.text}\n\nبرای استفاده پیام بده به ادمین 😊", parse_mode="Markdown")
...
async def search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"🔍 *جستجوی محصول*\n\nکانال ما رو چک کن 👇\n{CHANNEL_LINK}\n\nهر محصولی خواستی لینک یا عکسش رو بفرست 🛍️\n\n💡 «موجود در ایران» = الان موجود\nبدون این نوشته = سفارشیه",
        parse_mode="Markdown", reply_markup=main_keyboard()
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔙 خروج از پنل": return await exit_admin(update, context)
